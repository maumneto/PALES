...
void imprime_listavizinhossemanticos (ListaVizinhosSemanticos l)
{
	...
	for (p = l; p != NULL; p = p->prox)
	{
		printf("No = %d\ CH = %d Fuzzy= %d\n", p->id,p->CH,p->fuzzySaida);
	}
}
...
uint8_t get_quantidade_CH_semantico(ListaVizinhosSemanticos l)
{
	ListaVizinhosSemanticos p;
	uint8_t cont = 0,CH = l->CH;
	for (p = l; p != NULL; p = p->prox)
	{
		if(p->CH != CH)
			cont++;
...
void atualiza_vizinho(ListaVizinhosSemanticos l, uint8_t CH, uint8_t fuzzy)
{
	ListaVizinhosSemanticos p;
	for (p = l; p != NULL; p = p->prox)
	{
